...
class NPC:
    def __init__(self, name):
        self.name = name
        self.topics = NPCTopics()   # each NPC gets its own topic tracker

    def get_topics(self):
        topics = []

        if "family" not in self.topics.topics_seen:
            topics.append(("Ask about their family", "family"))

        if "road" not in self.topics.topics_seen:
            topics.append(("Ask about the road", "road"))

        if "bridge" not in self.topics.topics_seen:
            topics.append(("Ask about the bridge", "bridge"))

        if "feeling" not in self.topics.topics_seen:
    ...
    def respond(self, topic_key):
        self.topics.topics_seen.add(topic_key)
