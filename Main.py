...
def run_lucid_night(npcs):
    ...
    while running:
        ...
        if stage == 0:
            ...
            text = (
                f"{chosen.name} looks past you, at the empty air.\n"
                "Their breathing is too sharp, too real.\n\n"
...
def run_horror_chapter():
    ...
    intro_text = (
        "You open your eyes.\n"
        "The wagon is still moving.\n"
        "The bridge is much closer now.\n"
        "The place where someone sat is empty.\n\n"
        "You remember being woken in the night.\n"
        "You remember a voice asking for help.\n\n"
        "Press any key."
    )
    ...
    while horror_running:
        ...
        if phase == "intro":
            draw_text(screen, intro_text, font_main, TEXT_COLOR_HORROR, LEFT_COL, 80)
...
def main():
    ...
    if phase_result == "death":
        screen.fill(BG_COLOR_NORMAL)
        draw_text(screen, "Your journey ends here.", font_big, TEXT_COLOR, WIDTH // 2, HEIGHT // 3, center=True)
        draw_text(screen, info, font_small, TEXT_COLOR_DIM, LEFT_COL, HEIGHT // 2)
        draw_text(screen, "Press any key to exit.", font_small, TEXT_COLOR_DIM, LEFT_COL, HEIGHT // 2 + 80)
        draw_scanlines(screen)
        pygame.display.flip()
        wait_for_key()
        pygame.quit()
